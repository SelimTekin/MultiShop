    ...
    public class FeatureController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<List<ResultFeatureDto>>(jsonData); // json formatını metne çevir
                return View(values);
            }
            return View();
        }
        ...
        public async Task<IActionResult> CreateFeature(CreateFeatureDto createFeatureDto)
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "Feature", new { area = "Admin" }); // Admin areadaki Feature controller'ının Index sayfasına yönlendir
            }
            return View();
        }
        ...
        public async Task<IActionResult> UpdateFeature(string id)
        {
            ...
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync(); // gelen veriyi string formatta oku
                var values = JsonConvert.DeserializeObject<UpdateFeatureDto>(jsonData); // json formatını metne çevir
                return View(values);
            }
            return View();
        }
